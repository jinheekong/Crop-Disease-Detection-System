...
![Image](https://github.com/user-attachments/assets/7cf1fc96-b5ea-4f22-8876-4f4d7b92bc9a)
 ...
 윈도우 환경에서 코드를 작성하여 윈도우 명령창을 통해 flask 서버를 실행시켰습니다. flask 서버는 AI model이 포함되게 작성했으며, 해당 서버에서 라즈베리파이에서 찍은 사진을 전송받고, 전송받은 사진을 서버에 포함된 AI model을 이용하여 병해충 감염 유무를 판별하게 하였습니다. 또한, flask 서버를 구축한 환경과 다른 wifi에 연결되어있어도 라즈베리파이에서 flask 서버에 접근할 수 있도록 ngrok을 이용하여 서버의 접근성을 높였습니다.
...
```
 ...
 App Inventer를 활용하여 스마트폰 어플리케이션을 제작했습니다. 앱과 서버의 연동이 잘 이루어지지 않아 앱에서 병해충 탐지 결과를 확인 하는 것에는 실패하였지만, flask 서버 코드 또는 App Inventer 블록코딩을 조금만 수정하면 문제가 해결될 것이라고 생각합니다.
